...
//        封装VO返回结果
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        if (payStatus != null && payStatus.equals(Orders.PAID)) {
            ...
            if (Boolean.TRUE.equals(weChatProperties.getMockPay())) {
                // 模拟支付模式下，跳过真实微信退款调用
                log.info("[模拟支付] 拒单退款：订单号：{}，跳过真实退款", ordersDB.getNumber());
            } else {
    ...
    public void cancel(OrdersCancelDTO ordersCancelDTO) throws Exception {
        ...
        if (payStatus != null && payStatus.equals(Orders.PAID)) {
            ...
            if (Boolean.TRUE.equals(weChatProperties.getMockPay())) {
                // 模拟支付模式下，跳过真实微信退款调用
                log.info("[模拟支付] 取消订单退款：订单号：{}，跳过真实退款", ordersDB.getNumber());
            } else {
                String refund = weChatPayUtil.refund(
                        ordersDB.getNumber(),
                        ordersDB.getNumber(),
    ...
    private void checkOutOfRange(String address) {
        log.info("开始校验地址配送范围，校验地址：{}，店铺地址：{}", address, shopAddress);
        Map map = new HashMap();
        ...
        log.info("用户收货地址解析结果：{}", userCoordinate);
        ...
        if(distance > 5000){
            //配送距离超过5000米
            log.warn("地址[{}]超出配送范围，配送距离：{}米", address, distance);
            throw new OrderBusinessException("超出配送范围");
        }
        log.info("地址[{}]在配送范围内，配送距离：{}米", address, distance);
    }
